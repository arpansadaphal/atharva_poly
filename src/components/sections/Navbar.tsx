...
//NAVBAR 222222222222222222222222.   NAVBAR
...
// NAVBAR 333333333333333333333333.   NAVABAR 333333333333333333333333.   NAVABAR 333333333333333333333333.   NAVABAR 333333333333333333333333.   NAVABAR 3333333333333333333
...
import { cn } from '@/lib/utils'
import { navigation, company } from '@/lib/data'
import { megaMenuData, type MegaIcon } from '@/lib/navigation'
import { buildWhatsAppURL } from '@/lib/whatsapp'
import { navbarReveal, mobileMenuReveal, mobileNavItem } from '@/lib/animations'

const FOCUS =
  'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2'

// Brand easing curve — mirrors `ease` in lib/animations. Duplicated as a
// Tailwind-safe literal so it can be dropped into arbitrary `ease-[...]`
// classes as well as Framer Motion `transition` objects.
const EASE = [0.22, 1, 0.36, 1] as const
const EASE_CSS = 'cubic-bezier(0.22,1,0.36,1)'

// Static brand wordmark subtitle — not page content, so it lives here
// rather than in lib/data.ts. Replaces an earlier unsafe `as` cast onto
// `company` for a field the type didn't declare.
const COMPANY_SUBTITLE = 'POLYMERS PVT LTD'

// 'Solutions' is intentionally left out of the visible primary nav for now.
// It stays in lib/data.ts `navigation.primary` so it can be reinstated (as
// a regular link or folded into the mega menu) without a data change —
// see handoff notes for the open decision.
const HIDDEN_NAV_LABELS: string[] = ['Solutions']
 ...
 * Navbar — always-white, mega-menu-equipped navigation shell.
 *
 * - Fixed, full-width, z-50, always white (deliberate — matches the logo).
 *   Separation from content is handled with a hairline border + shadow that
 *   both deepen slightly on scroll, rather than a background-color change.
 * - Desktop: logo | nav links (centered) | divider | WhatsApp icon + Request Quote
 * - Products link navigates to /products; hover/click reveals the mega menu
 * - Mobile (<1024px): logo | hamburger → full-screen dark overlay with
 *   a Products accordion
 * - Body scroll locked while the mobile menu is open
 * - Mobile menu + mega menu close automatically on route change
 */
export function Navbar() {
  const [menuOpen, setMenuOpen] = useState(false)
  const [megaOpen, setMegaOpen] = useState(false)
  const [mobileAccordionOpen, setMobileAccordionOpen] = useState(false)
  // Scroll state is used ONLY to deepen the hairline border / shadow — the
  // background stays white at all times, per the locked design decision.
  const [scrolled, setScrolled] = useState(false)
  const pathname = usePathname()
  ...
  /* ── Scroll detection — shadow/border refinement only ───────────────── */
  useEffect(() => {
  ...
  //   if (typeof window !== 'undefined' && window.innerWidth < 1024) return
  //   // Toggle the mega menu without preventing navigation — clicking still
  //   // takes the user to /products; this just governs hover-open state.
  //   if (megaOpen) {
  //     closeMega(true)
  //   } else {
  //     openMega()
...
const handleTriggerClick = () => {
  if (typeof window !== 'undefined' && window.innerWidth < 1024) return
  // Only close if the mega menu is open; do not toggle open on click.
  // Navigation to /products will close the menu via the route change effect
  // if it was somehow left open.
  if (megaOpen) {
    closeMega(true)
  }
}
  ...
  const productsNavItem = navigation.primary.find(isProductsItem)
  const regularNavItems = navigation.primary.filter(
    (item) => !isProductsItem(item) && !HIDDEN_NAV_LABELS.includes(item.label),
  )

  return (
    <>
      {/* ── Fixed Header (always white — separation via border/shadow only) ── */}
      <motion.header
        className="fixed top-0 left-0 right-0 z-50 h-[72px] bg-white border-b"
        style={{
          borderColor: scrolled ? 'rgba(226,232,240,0.9)' : 'rgba(226,232,240,0.5)',
          boxShadow: scrolled
            ? '0 4px 20px rgba(15,23,42,0.06)'
            : '0 1px 2px rgba(15,23,42,0.03)',
          transition: `box-shadow 300ms ${EASE_CSS}, border-color 300ms ${EASE_CSS}`,
        }}
        initial="hidden"
        animate="visible"
        variants={navbarReveal}
      >
        <div className="max-w-[1280px] mx-auto px-6 lg:px-12 h-full flex items-center justify-between">
          ...
          <Link
            href="/"
            className={cn('inline-flex items-center gap-[10px] flex-shrink-0 mr-8 lg:mr-10 rounded-lg', FOCUS)}
            aria-label={`${company.name} — Return to homepage`}
          >
            <Image
              src="/assets/logos/croplogo.png"
          ...
          >
            ...
            {regularNavItems.map((item) => (
              <Link
                ...
                href={item.href}
                className={cn(
                  'relative inline-flex items-center px-4 py-2 text-[15px] font-medium rounded-md',
              ...
              >
                ...
                <span
                  aria-hidden="true"
                  className={cn(
                    'absolute bottom-[6px] left-4 right-4 h-[2px] bg-blue-600 rounded-full origin-left',
            ...
            {productsNavItem && (
              <Link
                ...
                id="products-trigger"
                className={cn(
                  'relative inline-flex items-center gap-[6px] px-4 py-2 text-[15px] font-medium rounded-md',
                  'transition-colors duration-200',
                  FOCUS,
                  megaOpen || isActive(productsNavItem.href)
                    ? 'text-blue-600'
          ...
          <div className="hidden lg:flex items-center gap-4 flex-shrink-0">
            {/* Precision divider — same 1px language as the hero trust row */}
            ...
            <a
              ...
              rel="noopener noreferrer"
              className={cn(
                'flex items-center justify-center w-9 h-9 rounded-full text-slate-500',
            ...
            <Link
              ...
              className={cn(
                'h-[38px] px-5 bg-blue-600 text-white text-[15px] font-semibold rounded-lg',
                'inline-flex items-center justify-center flex-shrink-0',
                'shadow-[0_1px_2px_rgba(37,99,235,0.15)]',
                'hover:bg-blue-700 hover:shadow-[0_6px_16px_rgba(37,99,235,0.22)]',
                'active:scale-[0.97] active:shadow-[0_1px_2px_rgba(37,99,235,0.15)]',
          ...
          <button
            className={cn(
              'lg:hidden flex flex-col gap-[5px] bg-transparent border-none cursor-pointer p-[8px] rounded-lg z-[60] relative',
      ...
      <AnimatePresence>
        {megaOpen && (
          <motion.div
            id="mega-panel"
            className="hidden lg:block fixed top-[72px] left-0 right-0 z-40 bg-white border-b border-slate-100"
            style={{
              boxShadow: '0 16px 40px -8px rgba(15,23,42,0.10)',
              transformOrigin: 'top',
            }}
            ...
            aria-label="Products navigation"
          >
            <div className="max-w-[1280px] mx-auto px-12 py-11 grid grid-cols-[1.15fr_0.85fr_1fr]">
              {/* Column 1: Product Categories */}
              <div className="pr-10">
                <div className="flex items-center gap-[10px] mb-5">
                  <span className="w-0.5 h-3 bg-blue-600 flex-shrink-0" aria-hidden="true" />
                  <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.1em]">
                    Product Portfolio
                  </span>
                </div>
                {megaMenuData.categories.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group flex items-start gap-3 px-3 py-[10px] -mx-3 rounded-lg hover:bg-slate-50 transition-colors duration-150"
                    onClick={() => setMegaOpen(false)}
                  >
                    <span
                      className="flex-shrink-0 w-[18px] h-[18px] text-blue-600 mt-[2px] transition-transform duration-200 group-hover:translate-x-0.5"
                      style={{ transitionTimingFunction: EASE_CSS }}
                    >
                      {megaIcon(item.icon)}
                    </span>
                    <div className="flex flex-col">
                      <span className="text-[14px] font-semibold text-slate-900">{item.label}</span>
                      <span className="text-[12.5px] text-slate-500 mt-[2px] leading-[1.5]">{item.desc}</span>
                    </div>
                  </Link>
                ))}
                <div className="border-t border-slate-100 mt-4 pt-3">
                  <Link
                    href="/products"
                    className="group inline-flex items-center gap-[6px] text-[14px] font-medium text-blue-600 px-3 py-[6px] -mx-3 rounded-md hover:bg-slate-50 transition-colors duration-150"
                    onClick={() => setMegaOpen(false)}
                  >
                    View All Products
                    <ArrowRight
              ...
              {/* Column 2: By Industry */}
              <div className="px-10 border-l border-slate-100">
                <div className="flex items-center gap-[10px] mb-5">
                  <span className="w-0.5 h-3 bg-blue-600 flex-shrink-0" aria-hidden="true" />
                  <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.1em]">
                    By Industry
                ...
                {megaMenuData.industries.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex items-center gap-3 px-3 py-2 -mx-3 rounded-lg hover:bg-slate-50 transition-colors duration-150 text-[14px] font-medium text-slate-700 hover:text-blue-600"
                    onClick={() => setMegaOpen(false)}
                  >
                    <span className="flex-shrink-0 w-4 h-4 text-slate-400 transition-colors duration-150">
                      {megaIcon(item.icon)}
              ...
              {/*
                Column 4 (CTA panel) intentionally left out — was previously
                commented out in the source. If it's reinstated, switch the
                grid template above to 4 columns (e.g.
                grid-cols-[1fr_0.8fr_0.9fr_0.9fr]) — the 3-column template
                is fitted to exactly the 3 rendered columns so there's no
            ...
            <div className="relative flex flex-col min-h-full px-6 py-6 max-[640px]:px-4 max-[640px]:py-4">
                  ...
                  >
                    <Link
                      ...
                      className={cn(
                        'flex items-center py-[14px] text-2xl max-[640px]:text-xl font-medium transition-colors duration-150',
                        FOCUS,
                        isActive(item.href) ? 'text-blue-400' : 'text-white hover:text-slate-300',
                      )}
                    >
                      {isActive(item.href) && (
                        <span className="w-[3px] h-6 bg-blue-600 mr-4 flex-shrink-0 rounded-[2px]" aria-hidden="true" />
                  ...
                  >
                    <div className="flex items-center justify-between w-full py-[14px]">
                      <Link
                        ...
                        className={cn(
                          'text-2xl max-[640px]:text-xl font-medium transition-colors duration-150',
                          isActive(productsNavItem.href) ? 'text-blue-400' : 'text-white hover:text-slate-300',
                        )}
                      >
                        {isActive(productsNavItem.href) && (
                          <span
                            className="w-[3px] h-6 bg-blue-600 mr-4 flex-shrink-0 rounded-[2px] inline-block align-middle"
                            aria-hidden="true"
                          />
                        )}
                        {productsNavItem.label}
                      ...
                      <button
                        className={cn(
                          'text-[14px] text-slate-500 bg-transparent border-none cursor-pointer p-2 rounded-md',
                          FOCUS,
                        )}
                        onClick={(e) => {
                          e.preventDefault()
                        ...
                        aria-label={mobileAccordionOpen ? 'Collapse products menu' : 'Expand products menu'}
                      >
                        <motion.span
                          animate={{ rotate: mobileAccordionOpen ? 180 : 0 }}
                          transition={{ duration: 0.2, ease: EASE }}
              ...
              >
                <Link
                  ...
                  className={cn(
                    'w-full h-14 bg-blue-600 text-white text-[16px] font-semibold rounded-lg',
                    'hover:bg-blue-700 active:scale-[0.98] transition-all flex items-center justify-center',
                    FOCUS,
                  )}
                  onClick={() => setMenuOpen(false)}
                >
                ...
                <a
                  ...
                  className={cn(
                    'w-full h-14 border-2 border-slate-700 text-white text-[16px] font-semibold rounded-lg',
                    'hover:border-slate-500 hover:bg-white/5 active:scale-[0.98] transition-all flex items-center justify-center gap-2',
                    FOCUS,
                  )}
                >
                  <MessageCircle size={20} aria-hidden="true" />
