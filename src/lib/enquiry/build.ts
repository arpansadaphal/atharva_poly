...
/**
 * Builds the CRM‑ready Enquiry object.
 *
 * `submission` is the validated business data from the form. `extraDetails`
 * is an optional bag of additional information (like resume file metadata)
 * that should be merged into the enquiry's `details` map without appearing
 * in the form schema or being part of `ContactSubmission`.
 */
export function buildEnquiry(
  { submission, page, referrer, prefill, ip, userAgent }: BuildEnquiryInput,
  extraDetails?: Record<string, string | number | undefined>,
): Enquiry {
  const details: Record<string, string> = {}

  // Extract all non‑core fields from the submission.
  for (const [key, value] of Object.entries(submission)) {
    if (CORE_FIELD_KEYS.has(key)) continue
    if (value === undefined || value === null || value === '') continue
    details[key] = String(value)
  ...
  // Merge any extra details (e.g. resumeFileName, resumeFileSize) after
  // the form fields, so they appear at the end of the details object.
  if (extraDetails) {
